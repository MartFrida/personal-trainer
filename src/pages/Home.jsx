  ...
  return (
    <ContainerMain>
      <section className="relative rounded-lg">
        <div
          className=" flex justify-center flex-col bg-fixed bg-no-repeat bg-top bg-cover md:bg-contain h-screen mt-[-50px] mb:mt-[-20px]"
        ...
        >
          <div className="bg-zinc-900/60 p-4 md:p-8 rounded-lg">
            ...
            <h2
              className="text-2xl md:text-3xl font-bold leading-relaxed tracking-wide text-center text-white"
              style={{textShadow: '4px 4px 10px rgba(0, 0, 0, 0.9)'}}
            >
              {t ('home.title')}
            ...
            <p
              className="text-xl md:text-2xl mt-4 text-center text-white"
      ...
      <div className={`${theme.text} flex flex-col gap-4`}>
        ...
        <SectionRegular>
          <h2
            className={`${theme.primary} text-2xl p-4 rounded-lg hover:scale-101 transition duration-300`}
          >
            {t ('home.offer.title')}
          </h2>
          <ul className="list-disc pl-6 mt-4 md:my-6">
            {listOffer.map ((item, index) => <li key={index}>{item}</li>)}
          </ul>
        </SectionRegular>
        ...
        <SectionRegular>
          ...
          <div className="flex flex-col lg:flex-row my-4 gap-4 ">
            <ul className=" list-inside list-decimal my-4 md:pl-6 list-none">
              {listPorque.map ((item, index) => <li key={index}>✔️ {item}</li>)}
            </ul>
        ...
        <SectionRegular className="mt-8">
          <h2 className={`${theme.primary} text-2xl p-4 rounded-lg`}>
            {t ('home.programas.title')}
          </h2>
          <div className="flex flex-col lg:flex-row my-4 gap-4">
            <div>
              <h3 className="text-xl font-semibold py-4 ">
                {t ('home.programas.part1.subtitle')}
              </h3>
              <p>{t ('home.programas.part1.listDescription')}</p>
              <ul className="list-inside list-disc md:pl-6 list-none">
                {t ('home.programas.part1.list', {
                  returnObjects: true,
                }).map ((item, index) => <li key={index}>✔️ {item}</li>)}
              </ul>

              <h3 className="text-xl font-semibold py-4 ">
                {t ('home.programas.part2.subtitle')}
              </h3>
              <p>{t ('home.programas.part2.listDescription')}</p>
              <ul className="list-inside list-disc md:pl-6 list-none">
                {t ('home.programas.part2.list', {
                  returnObjects: true,
                }).map ((item, index) => <li key={index}>✔️ {item}</li>)}
              </ul>

              <h3 className="text-xl font-semibold py-4 ">
                {t ('home.programas.part3.subtitle')}
              </h3>
              <p>{t ('home.programas.part3.listDescription')}</p>
              <ul className="list-inside list-disc md:pl-6 list-none">
                {t ('home.programas.part3.list', {
                  returnObjects: true,
                }).map ((item, index) => <li key={index}>✔️ {item}</li>)}
              </ul>
            </div>
